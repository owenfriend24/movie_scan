#!/usr/bin/env python
"""
Adult-only LOSO classification of within- vs across-triplet pairs (Run=4 by default).

Features:
  - For each subject, create all item pairs (C(12,2)=66)
  - Feature vector = voxelwise absolute difference |β_i - β_j|, masked by group mask
  - Label: 1 = within same triplet, 0 = across different triplets
  - Optional per-voxel item-wise z-scoring before pairwise diffs

Classifier & CV:
  - Leave-One-Subject-Out (LOSO) across adults
  - Default: L2 Logistic Regression (class_weight='balanced')
  - Optional: Linear SVM
  - Reports overall accuracy, ROC-AUC, precision/recall/F1, and confusion matrix
  - Also writes per-subject accuracy and counts

Inputs:
  meta_csv: subject, age_group, run, item_id, triplet_id, beta_path
  mask_nii: group/top-% mask NIfTI (binary); features are restricted to voxels==1
  outdir:   where to write reports & artifacts

Example:
  python loso_within_across_adults.py meta.csv group_top5pct_GM_posz_mask.nii.gz /scratch/out/loso_report \
...
# ---------------------- args ---------------------- #
def get_args():
    p = argparse.ArgumentParser(description="Adult LOSO classifier for within vs across triplet pairs.")
    p.add_argument("meta_csv", help="CSV with: subject,age_group,run,item_id,triplet_id,beta_path")
    p.add_argument("mask_nii", help="Binary NIfTI mask (e.g., group top-% Wilcoxon z mask)")
    p.add_argument("outdir", help="Output directory")

    p.add_argument("--run", type=int, default=4, help="Run number to use (default: 4)")
    ...
    p.add_argument("--zscore_items", action="store_true",
                   help="Z-score the 12 item betas per voxel before forming pairwise diffs (default: off)")
    p.add_argument("--random_state", type=int, default=13, help="Random seed (for reproducibility)")
    return p.parse_args()
...
def pairs_features_and_labels(X_items, triplet_ids, zscore_items=False):
    ...
    X = X_items.copy()
    if zscore_items:
        # z-score across the 12 items per voxel (column-wise)
...
def make_classifier(kind, C, random_state):
    if kind == "logreg":
        ...
        pipe = Pipeline([("scaler", StandardScaler(with_mean=True, with_std=True)),
                         ("clf", clf)])
        return pipe, "prob"  # predict_proba available
    else:
        # Linear SVM; use decision_function for scores
        clf = LinearSVC(C=C, class_weight="balanced", max_iter=5000, random_state=random_state)
        pipe = Pipeline([("scaler", StandardScaler(with_mean=True, with_std=True)),
                         ("clf", clf)])
        return pipe, "dec"   # decision_function available
...
def main():
    ...
    # Load meta and filter to ADULTS & chosen RUN
    ...
    if adults.empty:
        raise ValueError("No adult rows found for the requested run.")

    # Load mask and flatten indices
    mask_img, mask_bool = load_mask(args.mask_nii)
    aff, hdr, shape = mask_img.affine, mask_img.header, mask_img.shape
    flat_mask_idx = gm_indices(mask_bool)
    Vmask = flat_mask_idx.size
    print(f"[INFO] Mask voxels: {Vmask}")
    ...
    for sid in subjects:
        ...
        if rows.shape[0] == 0:
            continue
        X_items = load_items_for_subject(rows, flat_mask_idx)  # (12, Vmask)
    ...
    for test_sid in subjects:
        ...
        y_pred = (y_score >= 0).astype(int) if score_mode == "dec" else (y_score >= 0.5).astype(int)
        ...
        per_subject_counts.append({"subject": test_sid,
    ...
    print(f"Classifier: {args.clf}  C={args.C}  zscore_items={args.zscore_items}")
    ...
    with open(os.path.join(args.outdir, "summary.txt"), "w") as f:
        ...
        f.write(f"Classifier: {args.clf}  C={args.C}\n")
    ...
    # Optional: save raw predictions/scores for any deeper analyses
